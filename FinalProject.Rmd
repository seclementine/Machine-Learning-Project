...
In this report I will be creating a model to predict 5 different exercises from the results of sensors on 6 participants. The data was taken from accelerometers on the belt, forearm, arm, and dumbell.
...
I found out really fast that there are a ton of NA values in the training set so I decided to eliminate those columns from the data set especially since there were very few values with data
```{r nas}
...
I removed the X variable since I don't want my results to rely on the X value. 
...
Here's the results from using the random forest model on the validation set.
...
In my project I performed cross validation using the trainControl function. With method = cv and number = 5, I performed a cross validation with 5 resampling iterations. I also used cross validation by comparing with other models.
